# Module 0.2: Problem-Solving Framework (Go Edition)
...
**Questions to Ask**:
- What is the range of input sizes? (e.g., "Can the slice be nil or empty?")
...
- What should be returned for invalid inputs? (e.g., "Should I return an error or a zero value?")
...
**Common Patterns**:
- Two Pointers (sorted slices, palindromes)
- Sliding Window (sub-slice/substring problems)
...
- O(1) lookup needed → HashMap over slice scan

Data Structure Choice: map[int]int
- Key: number value
- Value: index in slice
```
...
- Searching takes O(n) in slice, but O(1) in Map

Optimization:
"Instead of searching for the complement each time, I'll 
store numbers I've seen in a Map. For each number, I 
...
Initial state: m = map[int]int{}, i = 0
...
5. **Consistency**: Follow Go idioms (e.g., `camelCase`, error handling)
...
func ts(a []int, t int) []int {
    m := make(map[int]int)
    for i, v := range a {
        c := t - v
        if val, ok := m[c]; ok { return []int{val, i} }
        m[v] = i
    }
    return []int{}
}
...
- Use `gofmt` style (standard Go formatting)
...
Test Case 3: Edge case (no solution)
...
Time Complexity: O(n)
- Single pass through slice: n iterations
- Map operations (get, put, lookup): O(1) average
- Total: O(n)

Space Complexity: O(n)
- Map stores up to n elements in worst case
...
- If slice were sorted: Two-pointer technique, O(n) time, O(1) space
